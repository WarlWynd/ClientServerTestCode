...
public class AuthHandler {
    ...
    public void handleLogin(DatagramSocket socket, Packet in,
        ...
        if (user.isPresent()) {
            ...
            // Role flags come directly from MySQL. The client applies allowExternalAdmin/
            // allowExternalDev to suppress tabs for external connections. We include those
            // settings here so the client has them immediately at login without waiting for
            // a SERVER_SETTINGS broadcast.
            ...
            out.put("allowExternalAdmin",  settings.allowExternalAdmin());
            out.put("allowExternalDev",    settings.allowExternalDev());
            ...
            log.info("LOGIN  ok  user='{}' admin={} graphicsDev={} boardDev={} audioDev={} from {}:{}",
                    ...
                    user.get().isBoardDev(), user.get().isAudioDev(), addr.getHostAddress(), port);
        } else {
